...
def Match(cliente,advogados)
	...
	if @verificador == 0
		...
		puts "Achamos " + @NumAdv.to_s + " advogado(s) cadastrado(s) para você, Sr(a) " + cliente.NomeCliente
		puts "Eis a lista dos profissionais encontrados:"
		puts "\n"

		for j in 0..(@NumAdv - 1)													# Imprime todos os advogados compatíveis com o Match
			puts @AdvogadosDisponiveis[j]
			puts "\n"
		end	
	end
end
